...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (data?.slides && editingId) {
      const slide = data.slides.find((s) => s.id === editingId);
      if (slide) {
        setForm({
  ...
  }, [data?.slides, editingId]);

  useEffect(() => {
    if (aboutData?.about) {
      setAboutForm({
        mission: aboutData.about.mission,
        vision: aboutData.about.vision,
        teamIntro: aboutData.about.teamIntro ?? "",
        expertise: parseJsonArray(aboutData.about.expertise),
        coreValues: parseJsonArray(aboutData.about.coreValues),
      });
    }
  }, [aboutData?.about]);

  useEffect(() => {
    if (teamData?.members && editingMemberId) {
      const m = teamData.members.find((x) => x.id === editingMemberId);
      if (m) {
        setMemberForm({
          name: m.name,
          role: m.role,
  ...
  }, [teamData?.members, editingMemberId]);

  useEffect(() => {
    if (blogData?.posts && editingPostId) {
      const p = blogData.posts.find((x) => x.id === editingPostId);
      if (p) {
        setPostForm({
          title: p.title,
          slug: p.slug,
  ...
  }, [blogData?.posts, editingPostId]);

  useEffect(() => {
    if (careersData?.jobs && editingJobId) {
      const j = careersData.jobs.find((x) => x.id === editingJobId);
      if (j) {
        setJobForm({
          title: j.title,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...form,
      tags: form.tags.length ? form.tags : ["AI", "Automation"],
    ...
    if (res.ok) {
      setForm({ heading: "", subHeading: "", tags: [], backgroundImageUrl: "", isActive: true, displayOrder: 0 });
      setEditingId(null);
      await mutate();
    } else {
      const data = await res.json().catch(() => null);
      const issues = data?.issues?.map((i: { message?: string }) => i?.message).filter(Boolean).join(", ");
      setHeroError(issues || data?.error || "Could not save slide");
    }
  };
  ...
  const handleHeroFile = async (file: File) => {
    ...
    if (!res.ok) {
      setHeroError("Image upload failed. Try a smaller file.");
      return;
    }
    const json = (await res.json()) as { url?: string };
    if (json.url) setForm((f) => ({ ...f, backgroundImageUrl: json.url! }));
  };
  ...
  const handleAboutSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await apiFetch("/api/admin/about", {
      ...
      body: JSON.stringify({
        ...
        teamIntro: aboutForm.teamIntro,
        expertise: aboutForm.expertise,
        coreValues: aboutForm.coreValues,
      }),
    });
    if (res.ok) {
      await mutateAbout();
      ...
      const data = await res.json().catch(() => null);
      setAboutError(data?.error || "Could not save About content");
    }
  };
  ...
  const handleMemberSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json().catch(() => null);
      setTeamError(data?.error || "Could not save member");
    }
  };
  ...
  const handlePostSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json().catch(() => null);
      setBlogError(data?.error || "Could not save post");
    }
  };
  ...
  const handleBlogImageUpload = async (file: File) => {
    ...
    if (!res.ok) {
      setBlogError("Image upload failed.");
      return;
    }
    const json = (await res.json()) as { url?: string };
  ...
  const handleMemberImageUpload = async (file: File) => {
    ...
    if (!res.ok) {
      setTeamError("Image upload failed.");
      return;
    }
    const json = (await res.json()) as { url?: string };
  ...
  const handleJobSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json().catch(() => null);
      let errorMsg = data?.error || "Could not save job posting";
      if (data?.issues && Array.isArray(data.issues)) {
        const details = data.issues.map((i: { path?: string[]; message?: string }) => `${i.path?.join('.') || 'field'}: ${i.message}`).join(', ');
        errorMsg = `${errorMsg} - ${details}`;
      }
      setCareersError(errorMsg);
    }
  };
  ...
  const sortedSlides = useMemo(
    () => (data?.slides ? [...data.slides].sort((a, b) => a.displayOrder - b.displayOrder) : []),
    [data?.slides]
  );
  ...
  if (!isAuthed) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <p className="text-sm text-gray-200">Redirecting to login…</p>
      </div>
    );
  }
          ...
          {activeSection === "hero" && (
            <div className="space-y-8">
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-6">{editingId ? "Edit Slide" : "Create New Slide"}</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Heading</label>
                      <input value={form.heading} onChange={(e) => setForm((f) => ({ ...f, heading: e.target.value }))} placeholder="Enter heading" className="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition" required />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Tags (comma separated)</label>
                      <input value={form.tags.join(", ")} onChange={(e) => setForm((f) => ({ ...f, tags: e.target.value.split(",").map((t) => t.trim()).filter(Boolean) }))} placeholder="AI, Automation" className="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Sub Heading</label>
                    <textarea value={form.subHeading} onChange={(e) => setForm((f) => ({ ...f, subHeading: e.target.value }))} placeholder="Enter sub heading" rows={3} className="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition resize-none" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Background Image URL</label>
                    <input value={form.backgroundImageUrl} onChange={(e) => setForm((f) => ({ ...f, backgroundImageUrl: e.target.value }))} placeholder="https://..." className="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition" required />
                  </div>
                  <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-6">
                    <label className="flex items-center gap-2">
                      <input type="checkbox" checked={form.isActive} onChange={(e) => setForm((f) => ({ ...f, isActive: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 text-orange-500 focus:ring-orange-500" />
                      <span className="text-sm text-gray-700">Active</span>
                    </label>
                    <div className="flex items-center gap-2">
                      <label className="text-sm text-gray-700">Order:</label>
                      <input type="number" value={form.displayOrder} onChange={(e) => setForm((f) => ({ ...f, displayOrder: Number(e.target.value) }))} className="w-20 rounded-lg border border-gray-200 px-3 py-2 text-center" />
                    </div>
                  </div>
                  {heroError && <p className="text-sm text-red-600">{heroError}</p>}
                  <div className="flex gap-3 pt-2">
                    <button type="submit" className="px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold hover:-translate-y-0.5 transition shadow-lg shadow-orange-500/25">{editingId ? "Save Changes" : "Create Slide"}</button>
                    {editingId && <button type="button" onClick={() => { setEditingId(null); setForm({ heading: "", subHeading: "", tags: [], backgroundImageUrl: "", isActive: true, displayOrder: 0 }); }} className="px-6 py-3 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50 transition">Cancel</button>}
                  </div>
                </form>
              </div>
                ...
                {sortedSlides.map((slide) => (
                  <div key={slide.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row md:items-center gap-6">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setEditingId(slide.id)} className="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:border-orange-300 hover:text-orange-600 transition text-sm">Edit</button>
                      <button onClick={() => handleDelete(slide.id)} className="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:border-red-200 hover:text-red-600 transition text-sm">Delete</button>
                    </div>
                  </div>
                ))}
          ...
          {activeSection === "blog" && (
            <div className="space-y-8">
              ...
              <div className="grid gap-4">
                ...
                {!blogData?.posts?.length && <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center text-gray-500">No posts yet. Create one above.</div>}
              </div>
            </div>
          )}
          ...
          {activeSection === "careers" && (
            <div className="space-y-8">
              ...
              <div className="grid gap-4">
                ...
                {!careersData?.jobs?.length && <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center text-gray-500">No job postings yet. Create one above.</div>}
              </div>
            </div>
          )}
        </main>
