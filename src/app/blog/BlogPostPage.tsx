...
async function getBlogPost(slug: string): Promise<BlogPost | null> {
    ...
    const base = apiBase
        ? apiBase.replace(/\/+$/, "")
        : (import.meta.env.VITE_SITE_URL ?? "http://localhost:5001");
    try {
...
export default function BlogPostPage() {
    ...
    useEffect(() => {
        if (slug) {
            setLoading(true);
            Promise.all([getBlogPost(slug), getRelatedPosts(slug)]).then(([fetchedPost, fetchedRelated]) => {
                setPost(fetchedPost);
        ...
        return (
            <div className="min-h-screen bg-white">
                ...
                <div className="flex flex-col justify-center items-center h-[60vh] text-center px-6">
                    ...
                    <Link to="/blog" className="px-6 py-3 bg-orange-500 text-white rounded-xl font-semibold hover:bg-orange-600 transition">
                        Back to Blog
                    </Link>
                </div>
                <Footer />
                            ...
                            {relatedPosts.map((relatedPost) => (
                                ...
                                >
                                    ...
                                    <div className="p-6">
                                        ...
                                        >
                                            Read More
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 0 24" stroke="currentColor">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                                            </svg>
                                        </Link>
                                    </div>
